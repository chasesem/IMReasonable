...
- (void)audio_PCMtoMP3:(double)time
{
    ...
    [self deleteCafCache];
    NSLog(@"MP3转换结束");
    if (_delegate && [_delegate respondsToSelector:@selector(endConvertWithData:filepath:time:)]) {
        ...
        NSString *filenew = [NSString stringWithFormat:@"%@/%@/%@.caf",path,@"voice",timeString];
      

        NSData *voiceData = [NSData dataWithContentsOfFile:[self mp3Path]];
        [_delegate endConvertWithData:voiceData filepath:[NSString stringWithFormat:@"%@.caf",timeString] time:time];
        
        [[NSFileManager  defaultManager] moveItemAtPath:[self mp3Path] toPath:filenew error:nil];
    }
}
