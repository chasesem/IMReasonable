...
@interface PJTextView() <UITextViewDelegate>
...
- (void)layoutSubviews
{
    ...
    int x=8;
    int y=6;
    int width=self.frame.size.width-2*x;
    // 根据文字计算label的高度
    CGSize maxSize = CGSizeMake(width, MAXFLOAT);
    CGSize placehoderSize = [self.placehoder sizeWithFont:self.placehoderLabel.font constrainedToSize:maxSize];
    int height = placehoderSize.height;
    self.placehoderLabel.frame=CGRectMake(x, y, width, height);
}
